...
namespace SoftrigAchievements.Controllers
{
    public class AllAchievements
    {
        ...
        public static List<Achievement> GetAllAchievements(ClaimsPrincipal user, Context database)
        {
            var globalIdentity = user.FindFirstValue(ClaimTypes.NameIdentifier);
            var achievementForUsers = database.AchievementForUsers.Where(x => x.Achieved && x.User == globalIdentity).Include(x => x.Achievement).ToList();
